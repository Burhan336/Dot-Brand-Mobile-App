  ...
  useEffect(() => {
    const fetchBrandData = async () => {
      try {
        ...
        if (accessToken) {
          const apiUrl =
            "https://dotbrand-api.onrender.com/api/v1/multiadmin/brand/";
          const config = {
            headers: {
          ...
          console.log("BrandData:", response.data.payload.brands); // Add this line to log the fetched data
          setBrandData(response.data.payload.brands);
          setFilteredData(response.data.payload.brands);
          setLoading(false);
        } else {
...
export default MultiAdminManageBrands;
